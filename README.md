...
| **Safe** | Crisis text routes to a **human-help card — never a verse** (+ optional guardian alert). Two layers: a deterministic regex floor **plus** an additive LLM net for indirect phrasings — checked first, on every surface. | `providers/gloo.py` |
...
- **Two-layer safety (defense in depth).** A deterministic, phrasing-robust **regex floor** runs on the raw text — identical in mock and live, offline, before any model call — so known crises are caught instantly and the gate can never drift, refuse, or rate-limit. On top of it, an **additive LLM net** (Gloo, when live) catches risk expressed in novel or indirect ways no pattern matches (*"I've made all the arrangements," "after Friday none of this matters"*). Strictly additive: the regex short-circuits, either signal routes to the **human-help card — never a verse**, and any LLM error falls back to the regex — so the deterministic guarantee is never weakened, only extended. Recall validated across 14+ crisis phrasings incl. indirect, method-specific, and slang. *(`providers/gloo.py`)*
